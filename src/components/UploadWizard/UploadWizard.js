...
class UploadWizard extends Component {
    ...
    async componentDidMount() {
        ...
        const { partType, name, extension, objectURL } = data;
        ...
        try {
            ...
            const currentObjectParent = mainSceneManager.getParentObject(
                partType.id,
            );
            ...
            const parentAttachPointPosition = getParentAttachPointPosition(
                partType,
            );
    ...
    onNext = () => {
        const { currentWizardStep, partType, attachPointsToPlace } = this.state;
        ...
        switch (currentWizardStep) {
            ...
            case steps.ADJUST: {
                const attachPointsLeftToPlace =
                    attachPointsToPlace && attachPointsToPlace.length !== 0;
    ...
    renderWizardStep() {
        const { onWizardCanceled } = this.props;
        ...
        const currentAttachPoint =
            attachPointsToPlace.length !== 0 ? attachPointsToPlace[0] : null;
        const currentChildPartType = this.props.getChildPartTypeByAttachPoint(
            currentAttachPoint,
        );
            ...
            case steps.ADJUST_ATTACHPOINTS:
                return (
                    <AdjustAttachpoints
                        currentCategory={partType}
                        currentObjectChild={
                            currentObjectChildren[currentAttachPoint]
                        }
                        uploadedObjectGeometry={uploadedObjectGeometry}
                        position={position}
                        rotation={rotation}
