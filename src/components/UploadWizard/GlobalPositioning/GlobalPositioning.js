...
export default class GlobalPositioning extends Component {
    componentDidMount() {
        const { currentObjectParent, uploadedObjectGeometry } = this.props;

        const parentObject =
            currentObjectParent &&
            object3dFind(currentObjectParent, object => object.isMesh);
    ...
    render() {
        return (
            <div className={cn(commonStyles.wizardStep, styles.container)}>
                ...
                <h2 className={styles.title}>Is it in the correct place?</h2>
